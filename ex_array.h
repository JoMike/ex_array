...
//Erasing Helpers---------------------------------------------------------------------------------------
	...
	/*void slide_to_left(ptrdiff_t i_slide_size, ptrdiff_t i_slide_amount)*/
	void slide_to_left(ptrdiff_t i_slide_size, ptrdiff_t i_slide_amount);

	/*void slide_to_right(ptrdiff_t i_slide_size, ptrdiff_t i_slide_amount)*/
	void slide_to_right(ptrdiff_t i_slide_size, ptrdiff_t i_slide_amount);
...
ex_array<St, Alloc>::ex_array(const allocator_type& i_alloc)
	:M_front_buffer_ratio(DEFAULT_FRONT_BUFFER_RATIO), M_Alloc(i_alloc){default_initialization();}
...
void ex_array<St, Alloc>::expand(size_type i_size){
	...
	St* new_front = std::min(new_zero + static_cast<ptrdiff_t>(i_size*M_front_buffer_ratio)
...
void ex_array<St, Alloc>::make_space_in_back(ptrdiff_t i_desired_back_buffer){
	...
	if(back_buffer() < i_desired_back_buffer){
		ptrdiff_t extra_space_needed_in_back = safe_cast<ptrdiff_t>(i_desired_back_buffer - back_buffer());

		assert(front_buffer() >= extra_space_needed_in_back);

		ptrdiff_t slide_size = safe_cast<ptrdiff_t>(buffer()*M_front_buffer_ratio - back_buffer());
		slide_size = slide_size > extra_space_needed_in_back ? slide_size : extra_space_needed_in_back;

		slide_size = std::max(slide_size
			, static_cast<ptrdiff_t>(front_buffer()*M_slide_overshoot_val));
...
void ex_array<St, Alloc>::resize_front(size_type i_size
	...
	if(size() > i_size){
		...
		construct_range(M_front, M_frost + increase_amount,  irk_val);
	}
}
